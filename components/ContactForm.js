    ...
    return (
        <div className="max-w-lg mx-auto">
            <div id="form-container" className="w-full h-full rounded-xl px-4 lg:px-10 py-5 bg-[color:var(--ls-white)] shadow-inner">
                ...
                {
                    props.formData.statusCode != ""
                        ? (
                            <div className="flex flex-col justify-center py-4 text-center">
                                {
                                    props.formData.statusCode == 200
                                        ? <p> Vielen Dank für Ihre Nachricht. Wir melden uns demnächst! </p>
                                        : <p> {`Leider hat unser Formular einen technischen Fehler (Statuscode ${props.formData.statusCode}). Melden Sie sich gern telefonisch bei uns!`}</p>
                                }
                            </div>
                        )
                        : (
